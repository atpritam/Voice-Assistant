...
class LLMRecognizer:
    ...
    def _build_user_prompt(self, query: str, conversation_history: Optional[List[Dict]] = None) -> str:
        ...
        prompt_parts.extend([
            ...
            "- Track the booking state and what has been collected",
    ...
    def _get_selective_business_info(self, intent: str) -> str:
        ...
        if intent == "booking":
            info = {
                "room_types": self.res_info.get("room_types"),
                "booking": self.res_info.get("booking"),
            }
            ...
            info = {
                "hotel_amenities": self.res_info.get("hotel_amenities"),
                "services": self.res_info.get("services")
            }
    ...
    def _get_system_prompt(self, intent_patterns: Dict, recognized_intent: Optional[str] = None, original_conf: Optional[float] = None) -> str:
        ...
        intent_descriptions = """- general: Greetings, thanks, confirmations, chitchat, general inquiries
        - booking: Making NEW reservations, room booking, checking availability, modifying existing bookings
        - amenities_inquiry: Asking about hotel facilities, services, WiFi, parking, pool, gym, breakfast, pet policy
        - room_types: Asking about room options, prices, bed types, views, room features, upgrades
        - hours_location: Front desk hours, check-in/check-out times, hotel location, address, directions
        - cancellation_modification: Cancellation requests, policy questions, changing dates, refund inquiries
        - complaint: Problems with room/service, wrong room type, cleanliness issues, sarcastic complaints, facility issues"""

        if self.test_mode:
            prompt = f"""Intent classification for Hotel Booking System. Classify into: {', '.join(valid_intents)}\n{intent_descriptions}"""
    ...
    Booking flow:
    1. User selects room type (ask check-in/check-out dates if missing)
    2. Confirm number of guests
    3. Mention relevant amenities or services
    4. Get guest name
    5. Collect contact information
    6. Confirm booking and provide summary
